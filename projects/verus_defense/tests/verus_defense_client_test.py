...
@pytest.fixture()
def verus_defense_client(
    algorand_client: AlgorandClient, deployer: SigningAccount
) -> VerusDefenseClient:
    factory = algorand_client.client.get_typed_app_factory(
        VerusDefenseFactory, default_sender=deployer.address
    )
...
def test_says_hello(verus_defense_client: VerusDefenseClient) -> None:
    result = verus_defense_client.send.hello(args=("World",))
    assert result.abi_return == "Hello, World"


def test_simulate_says_hello_with_correct_budget_consumed(
    verus_defense_client: VerusDefenseClient,
) -> None:
    result = (
        verus_defense_client.new_group()
        .hello(args=("World",))
        .hello(args=("Jane",))
        .simulate()
    )
